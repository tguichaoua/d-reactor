...
/** @internal */
export type OnVoteUpdate<T, Return> = (element: VoteElement<T>, votes: ReadonlyArray<readonly User[]>) => Return;
...
export function reactorVote<T, R>(
    ...
    return reactorList<T, VoteResult<T> | R, VoteResult<T>>(channel, caption, list, options, getVoteResult, {
        ...internalOptions,
        ...{
            onCollect({ user, index }) {
                ...
                if (!users.includes(user)) {
                    users.push(user);
                    if (callbacks?.onAdd) {
                        const action = callbacks.onAdd({ value: list[index], users }, votes);
                        if (action) {
                            if ("submit" in action) return action.submit ? { value: getVoteResult() } : undefined;
                            else return action;
                        }
                    }
                }
            ...
            onRemove({ user, index }) {
                ...
                if (i !== -1) {
                    users.splice(i, 1);
                    if (callbacks?.onRemove) callbacks.onRemove({ value: list[index], users }, votes);
                }
            },
        },
    });
