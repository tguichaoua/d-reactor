...
export function reactorList<T, R, C = R>(
    ...
    return new Reactor<R, C>(message, [...emojis, ...buttonEmojis], options, onEnd, {
        ...internalOptions,
        ...{
            onCollect(params) {
                const btnIndex = buttonEmojis.indexOf(params.reaction.emoji.name);
                if (btnIndex !== -1) return buttons[btnIndex].action() ?? { remove: true };
                else
                    return internalOptions.onCollect
                        ? internalOptions.onCollect(
                              Object.assign(params, {
                                  index: emojis.indexOf(params.reaction.emoji.name),
                              }),
                          )
                        : undefined;
            },
